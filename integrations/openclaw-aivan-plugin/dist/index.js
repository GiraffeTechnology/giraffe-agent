...
async function aivanFetch(path, options = {}) {
...
export async function health() {
    ...
    return { ok: true, status: result.data?.status ?? "ok" };
}
/**
...
export async function getPendingDrafts(projectId) {
    ...
    const result = await aivanFetch(`/api/openclaw/drafts/pending?project_id=${encodeURIComponent(projectId)}`);
    if (!result.ok) {
        return { ok: false, error: result.error };
    }
 ...
 */
export async function approveDraft(draftId, approvedBy) {
    if (!draftId) {
